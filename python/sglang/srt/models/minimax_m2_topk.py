...
# -----------------------------------------------------------------------------
# TopK-ID dumping (for tuning_fused_moe_triton_sep.py --topk-ids-dir)
#
# Your sep tuner loads:
#   topk_ids_layer{i%58+3}_idx{i//58}.pt
#
# So you typically want files:
#   topk_ids_layer3_idx0.pt ... topk_ids_layer60_idx0.pt
# and (ideally) idx1 too.
#
# This hook writes:
#   topk_ids_layer{layer_id + TOPK_LAYER_OFFSET}_idx{set_idx}.pt
#
# Environment variables:
#   TOPK_IDS_DIR         : enable dumping to this directory (required)
#   TOPK_LAYER_OFFSET    : integer offset to add to layer_id (default 0; use 3 to match your tuner)
#   TOPK_MAX_SETS        : how many sets per layer (default 2 => idx0, idx1)
#   TOPK_MIN_TOKENS      : only dump if num_tokens >= this (default 4096)
# -----------------------------------------------------------------------------
def _maybe_dump_topk_ids(layer_id: int, topk_ids: torch.Tensor) -> None:
    out_dir = os.environ.get("TOPK_IDS_DIR")
    if not out_dir:
        return

    # Only dump from TP rank 0 to avoid multi-writer races.
    try:
        if get_tensor_model_parallel_rank() != 0:
            return
    except Exception:
        # If TP isn't initialized for some reason, be conservative and do nothing.
        return

    try:
        min_tokens = int(os.environ.get("TOPK_MIN_TOKENS", "4096"))
    except Exception:
        min_tokens = 4096

    # topk_ids is shape [num_tokens, topk]
    if topk_ids is None or topk_ids.numel() == 0:
        return
    if topk_ids.shape[0] < min_tokens:
        return

    try:
        layer_offset = int(os.environ.get("TOPK_LAYER_OFFSET", "0"))
    except Exception:
        layer_offset = 0

    try:
        max_sets = int(os.environ.get("TOPK_MAX_SETS", "2"))
    except Exception:
        max_sets = 2
    max_sets = max(1, max_sets)

    # Count per "file layer id" so we generate idx0, idx1, ...
    file_layer = layer_id + layer_offset

    if not hasattr(_maybe_dump_topk_ids, "_counts"):
        _maybe_dump_topk_ids._counts = {}  # type: ignore[attr-defined]

    counts = _maybe_dump_topk_ids._counts  # type: ignore[attr-defined]
    cur = int(counts.get(file_layer, 0))
    if cur >= max_sets:
        return
    counts[file_layer] = cur + 1

    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, f"topk_ids_layer{file_layer}_idx{cur}.pt")

    # Save CPU tensor for portability and to reduce GPU memory pressure.
    try:
        torch.save(topk_ids.detach().to("cpu"), path)
        logger.info(
            "Dumped topk_ids: layer=%s (file_layer=%s) idx=%s tokens=%s -> %s",
            layer_id,
            file_layer,
            cur,
            topk_ids.shape[0],
        ...
        logger.warning("Failed to dump topk_ids to %s: %s", path, e)
...
class MiniMaxM2MoE(nn.Module):
    ...
    def forward_normal(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # --- dump routing ids for sep tuner ---
        _maybe_dump_topk_ids(self.layer_id, topk_output.topk_ids)
    ...
    def op_select_experts(self, state):
        ...
        if router_logits is not None:
            ...
            # --- dump routing ids for sep tuner (TBO path) ---
            _maybe_dump_topk_ids(self.layer_id, state.topk_idx_local)
    ...
    def op_combine_b(self, state):
        ...
        if self.ep_size > 1:
            state.hidden_states_after_combine = self.experts.deepep_dispatcher.combine_b(
                tbo_subbatch_index=state.get("tbo_subbatch_index"),
            )
...
class MiniMaxM2Attention(nn.Module):
    ...
    def __init__(
        ...
        self.rotary_dim = getattr(config, "rotary_dim", self.head_dim)  # MiniMax uses rotary_dim=64
...
class MiniMaxM2ForCausalLM(nn.Module):
    ...
    def _pp_should_skip_param(self, param_name: str) -> bool:
        ...
        This avoids masking genuine mapping bugs while still allowing each PP stage
        to ignore weights for other stages when the checkpoint iterator feeds "all keys"
        to every rank.
        """
        pp_group = get_pp_group()
